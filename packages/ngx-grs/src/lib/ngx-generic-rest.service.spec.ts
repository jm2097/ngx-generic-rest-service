...
describe('NgxGenericRestService', () => {
	describe('config', () => {
		...
		it('should be created', inject([TestService], (testService: TestService) => {
	...
	describe('list', () => {
		...
		it(`should transform request's result to desired result when called with config.mapResponseFn`, inject(
			...
			(testService: TestService, httpMock: HttpTestingController) => {
				...
				testService
					.list({
						mapFn: (res) => res.map((entity: TestEntity) => ({ name: entity.foo, age: 18 })),
					})
					.subscribe((result) => {
		...
		it('should add headers to request when called with config.headers', inject(
			[TestService, HttpTestingController],
			(testService: TestService, httpMock: HttpTestingController) => {
				const headers = new HttpHeaders().set('x-foo', 'foo').set('x-bar', 'bar');
		...
		it('should add params to request when called with config.params', inject(
			...
			(testService: TestService, httpMock: HttpTestingController) => {
				...
				const request = httpMock.expectOne((req) => req.url === testService.url);
	...
	describe('add', () => {
		...
		it('should rewrite request URL when called with config.rewrite', inject(
			...
			(testService: TestService, httpMock: HttpTestingController) => {
				const dummyEntity: TestEntity = { foo: 'foo', bar: 999 };
				const dummyUrl = 'dummy-url';
				testService.add<TestEntity>(dummyEntity, { urlRewrite: dummyUrl }).subscribe();
				const request = httpMock.expectOne(dummyUrl);
				request.flush([]);
			},
		));
		...
		it('should add postfix to request URL when called with config.urlPostfix', inject(
			...
			(testService: TestService, httpMock: HttpTestingController) => {
				...
				const dummyPostfix = 'foo/0/bar/1';
				const expectedUrl = `${testService.url}/${dummyPostfix}`;
				testService.add<TestEntity>(dummyEntity, { urlPostfix: dummyPostfix }).subscribe();
				const request = httpMock.expectOne((req) => req.url === expectedUrl);
				request.flush([]);
			},
		));
	...
	describe('update', () => {
		...
		it('should return result from request', inject(
			...
			(testService: TestService, httpMock: HttpTestingController) => {
				...
				testService.update<TestEntity>(dummyId, dummyEntity).subscribe((result) => {
					expect(result).toEqual(expectedResult);
				});
		...
		it('should rewrite request URL when called with config.rewrite', inject(
			...
			(testService: TestService, httpMock: HttpTestingController) => {
				...
				testService.update<TestEntity>(dummyId, dummyEntity, { urlRewrite: dummyUrl }).subscribe();
		...
		it('should add headers to request when called with config.headers', inject(
			...
			(testService: TestService, httpMock: HttpTestingController) => {
				...
				const dummyEntity: TestEntity = { foo: 'foo', bar: 999 };
				const headers = new HttpHeaders().set('x-foo', 'foo').set('x-bar', 'bar');
				testService.update<TestEntity>(dummyId, dummyEntity, { headers }).subscribe();
		...
		it('should add params to request when called with config.params', inject(
			...
			(testService: TestService, httpMock: HttpTestingController) => {
				...
				const dummyEntity: TestEntity = { foo: 'foo', bar: 999 };
				const params = new HttpParams().set('foo', 'foo').set('bar', 'bar');
				testService.update<TestEntity>(dummyId, dummyEntity, { params }).subscribe();
	...
	describe('delete', () => {
		...
		it('should rewrite request URL when called with config.rewrite', inject(
			...
			(testService: TestService, httpMock: HttpTestingController) => {
				...
				testService.delete<TestEntity>(dummyId, { urlRewrite: dummyUrl }).subscribe();
			...
			(testService: TestService, httpMock: HttpTestingController) => {
				...
				testService
					.delete<string>(dummyId, {
						mapFn: (res) => (res.succeeded && res.result ? 'DELETED' : 'NOT_DELETED'),
					})
					.subscribe((result) => {
